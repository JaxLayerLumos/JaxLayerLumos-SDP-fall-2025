...
def get_eps_mu(materials, frequencies):
    ...
    # *** Unit Conversion Fix: Convert Hz input to GHz for material models ***
    ...
    # 3. Last Layer (PEC or Michielssen Material)
    if materials[-1] == "PEC":
        eps_last = jnp.zeros((1, num_frequencies), dtype=jnp.complex128) + jnp.inf
        mu_last = jnp.ones((1, num_frequencies), dtype=jnp.complex128)
    else:
        try:
            # Pass the GHZ array
            eps_last, mu_last = get_eps_mu_Michielssen(
                materials[-1:].astype(int), frequencies_GHz 
            )
        except:
            raise ValueError("Error calculating final non-PEC material properties.")
...
def get_eps_mu_Michielssen(material_indices, frequencies):
    ...
    eps_r = M_epsr[material_indices - 1, :]
    mu_r = M_mur[material_indices - 1, :]

    return eps_r, mu_r


def get_eps_mus_real_materials(material_indices, frequencies): # frequencies is in GHz
    ...
    for i in material_indices: 
        ...
        # Pass the GHz frequency array directly to the helper
        if material_data_entry['section']==1:
            eps_r, mu_r = getEpAndMu_12_1(frequencies, material_data_entry)
        elif material_data_entry['section']==4:
            eps_r, mu_r = getEpAndMu_12_4(frequencies, material_data_entry)
        elif material_data_entry['section']==6:
            eps_r, mu_r = getEpAndMu_12_6(frequencies, material_data_entry)
        elif material_data_entry['section']==7:
            eps_r, mu_r = getEpAndMu_12_7(frequencies, material_data_entry)
        elif material_data_entry['section']==8:
            eps_r, mu_r = getEpAndMu_12_8(frequencies, material_data_entry) 
        else:
...
def getEpAndMu_12_1(frequencies, material):
...
def getEpAndMu_12_6(frequencies, material):
    params = material.get('eps_params', {})
    B = params.get('B', 0.0)
    C = params.get('C', 0.0)
    D = params.get('D', 0.0)
